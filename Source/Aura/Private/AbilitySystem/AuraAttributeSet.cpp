...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0)
	{
		...
		else
		{
			...
			{
				// Activate ability if it has hit react tag
		...
		const bool bShouldBlock = UAuraAbilitySystemLibrary::IsBlockedHit(Props.EffectContextHandle);
		const bool bCriticalHit = UAuraAbilitySystemLibrary::IsCriticalHit(Props.EffectContextHandle);
		ShowFloatingText(Props, LocalIncomingDamage, bShouldBlock, bCriticalHit);
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	if (const FGameplayTag DebuffTag = GameplayTags.DamageTypesToDebuffs[DamageType]; DebuffTag.MatchesTagExact(GameplayTags.Debuff_Stun))
	{
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		...
		{
			AuraPlayerController->ShowDamageNumber(Props.TargetCharacter, Damage, bBlockedHit, bCriticalHit);
			return;
		}
